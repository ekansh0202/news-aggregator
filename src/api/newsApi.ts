...
const API_KEYS = {
    ...
    nyt: "QyD9vz3dGba5FjY21wTTvZfGpZxX8gVh",
    newsApiOrg: "331b67236fa6421ca8afb3165df0fcc8",
};
  

// Fetch news from newsApi
export const fetchNewsFromNewsApi = async (categories: string[] = [], sources: string[] = []) => {
  // Default categories if none are provided
  const defaultCategories = ["Technology", "Sport"];

  // Use default categories if no categories are passed
  const categoryUris = (categories.length > 0 ? categories : defaultCategories).map(
    (category) => `http://en.wikipedia.org/wiki/${category}`
  );

  // Construct dynamic query for sources if sources are provided
  const sourceUris = sources.map((source) => `http://en.wikipedia.org/wiki/${source}`);
    ...
    query: {
      $query: {
        $and: [
          ...
          ...sourceUris.length > 0 ? sourceUris.map((sourceUri) => ({ conceptUri: sourceUri })) : [],
          {
            lang: "eng",
          },
        ],
  ...
  export const fetchNewsFromGuardianApi = async (categories: string[] = []) => {
    try {
      ...
      return response.data.response.results.map((item: any) => ({
        ...item, sourceType: 'guardian', id: uuidv4()
      }))
    } catch (error) {
  ...
  export const fetchNewsFromNewsApiOrg = async (sources: string[] = []) => {
    try {
      ...
      return response.data.articles.map((item: any) => ({
        ...item, sourceType: 'newsApiOrg', id: uuidv4()
      }))
    } catch (error) {
